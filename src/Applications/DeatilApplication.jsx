...
const handleAcceptAndReject= async(id,action)=>{
  try {
    const response=await axios.put(`https://backend-internarea-bs4w.onrender.com/api/application/${id}`,{action})
      ...
      <div class="container px-5 py-24 mx-auto">
        <div class="lg:w-4/5 mx-auto flex flex-wrap">
          ...
          <h2>{t('UserApplication:cover_letter')}</h2>
            ...
            <h4 className=' mt-9'>{t('UserApplication:company_name')}Applied By</h4>
     <p className='font-bold -mt-8'>{data.user.name}</p>
     <div className="flex mt-24 justify-around">
            <button className='bg-blue-700 text-green-400 w-24 font-bold' onClick={()=>handleAcceptAndReject(data._id,"accepted")}>{t('UserApplication:accept')}</button>
            <button className='bg-blue-700 text-red-600 w-24 font-bold' onClick={()=>handleAcceptAndReject(data._id,"rejected")}>{t('UserApplication:reject')}</button>
          </div>
          </div>
