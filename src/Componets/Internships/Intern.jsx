...
<p id='filter-ico' className=' text-center' ><i onClick={showDiv} class="bi bi-funnel  text-blue-400"></i> {t('internAndJob:filter')}</p>
...
<p>{t('internAndJob:desired_stipend')} (₹)</p>
...
<div className="search-2"><div className="search-container">
  ...
  <div className="all-internships">
    <div className=" show show2 flex justify-center">
      <p className='filterico text-center' onClick={showDiv}>{t('internAndJob:filter')} <i class="bi bi-funnel  text-blue-400"></i>   </p>

    </div>
    <p className='head font-bold text-lg text-center '  >{filterInternship.length} {t('internAndJob:total_internships')}</p>
...
<div className="flex text-sm justify-between">
  ...
  <p className='mt-3'> <i class="bi bi-calendar-check-fill"></i>  {t('internAndJob:duration')}  <br />
  {data.Duration}</p>

  <p className='mt-3'>  <i class="bi bi-cash"></i>   {t('internAndJob:stipend')} <br /> {data.stipend}</p>
   </div>
   </div>
   <span className='bg-slate-200 text-slate-400 w-20 rounded-sm text-center'>{t('internAndJob:internship')}</span>
   <br />
   <span><i class="bi bi-stopwatch text-green-300"></i>23/11/2065</span>
   <div className="flex justify-end" id='hr'>
<Link to={`/detailInternship?q=${data._id}`} className='mt-10'><button id='viewButtons' className='bg-transparent text-blue-500'>{t('internAndJob:view_deatils')}</button></Link>
   </div>
  </div>
  </div>
